#!/usr/bin/env python3
"""
Boston, MA Coverage Map Dataset Generator

Dense coastal urban area - generates path gain and elevation maps by dynamically 
...
# Boston, MA bounds (dense coastal urban area)
CITY_NAME = "boston"
CITY_LAT = (42.32, 42.40)
CITY_LON = (-71.12, -71.00)
...
PLOT = False
MIN_BUILDINGS = 10           # Dense urban - require more buildings
...
def generate_sample(args):
    ...
    margin_lon = radius / (111320.0 * np.cos(np.radians(np.mean(CITY_LAT))))
    
    lat = np.random.uniform(CITY_LAT[0] + margin_lat, CITY_LAT[1] - margin_lat)
    lon = np.random.uniform(CITY_LON[0] + margin_lon, CITY_LON[1] - margin_lon)
    ...
    try:
        ...
        # Check elevation range (skip extreme terrain)
        ...
        with h5py.File(output_file, 'w') as f:
            f.attrs.update(center_lat=lat, center_lon=lon, radius=radius, frequency=FREQUENCY,
                          ...
                          city=CITY_NAME, terrain_type="dense_mountain")
...
def main():
    ...
    print("=" * 60)
    print(f"Terrain type: Dense Mountain Urban")
